...
def test_engenheiro_inicializacao_crea_vazio(endereco_valido):
    with pytest.raises(ValueError):
        Engenheiro("João Silva", "9999-9999", "", "joao@example.com", endereco_valido, 5000.0)

def test_engenheiro_inicializacao_crea_incorreto_tamanho(endereco_valido):
    with pytest.raises(ValueError):
        Engenheiro("João Silva", "9999-9999", "123456", "joao@example.com", endereco_valido, 5000.0)

def test_engenheiro_inicializacao_crea_incorreto_numeros(endereco_valido):
    with pytest.raises(ValueError):
